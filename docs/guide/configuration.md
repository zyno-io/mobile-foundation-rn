...
| `MUS_URL` | Updater | Base URL of the Mobile Update Server (native-update status + MUS-targeted OTA) |
...
The three `MUS_*` keys are required together for [native (app-store) update checks](../api/updater#native-app-store-updates) and MUS-targeted OTA request headers. If any of them is missing, native-update checking is disabled (a no-op) and the related `Updater` accessors return their empty values.
